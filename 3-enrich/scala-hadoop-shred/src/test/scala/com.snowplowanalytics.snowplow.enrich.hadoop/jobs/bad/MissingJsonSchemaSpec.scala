...
object MissingJsonSchemaSpec {
  ...
  val expected = (line: String) => """{"line":"%s","errors":[{"level":"error","message":"Could not find schema with key iglu:com.snowplowanalytics.website/fake_context/jsonschema/1-0-0 in any repository, tried:","repositories":["Digdeep [HTTP]","Iglu Client Embedded [embedded]","Iglu Central [HTTP]"]}]}""".format(line.replaceAll("\"", "\\\\\"")).replaceAll("[\t]","\\\\t")
}
